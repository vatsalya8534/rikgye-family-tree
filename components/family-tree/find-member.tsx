...
export default function FindMember() {
    ...
    const filteredMembersFunc = async () => {
        ...
        let filterData = members.filter((m) => {
            const city = mode === "birth" ? m.birthPlace : m.currentResidence;

            if (!city) return false;
            if (!citySearch) return true; // show all if search is empty

            return city.toLowerCase().includes(citySearch.toLowerCase().trim());
        });
